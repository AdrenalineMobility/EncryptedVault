...
(function() {
  var DbxChromeOnMessage, DbxChromeSendMessage, DbxClient, DbxEnvGlobal, DbxEnvRequire, DbxXhrArrayBufferView, DbxXhrCanSendForms, DbxXhrDoesPreflight, DbxXhrIeMode, DbxXhrRequest, DbxXhrSendArrayBufferView, DbxXhrWrapBlob, Dropbox,
  ...
  if (typeof chrome !== "undefined" && chrome !== null) {
    ...
    if (chrome.extension) {
      if (chrome.extension.onMessage) {
        DbxChromeOnMessage || (DbxChromeOnMessage = chrome.extension.onMessage);
      }
      if (chrome.extension.sendMessage) {
        DbxChromeSendMessage || (DbxChromeSendMessage = chrome.extension.sendMessage.bind(chrome.extension));
      }
    }
  }
  ...
  Dropbox.AuthDriver.Chrome = (function(_super) {
    ...
    Chrome.prototype.onMessage = DbxChromeOnMessage;

    Chrome.prototype.sendMessage = DbxChromeSendMessage;

    Chrome.prototype.expandUrl = function(url) {
      if (chrome.runtime.id) {
        return 'https://' + chrome.runtime.id + '.chromiumapp.org/' + url;
      }
  ...
  Dropbox.Client = (function() {
    function Client(options) {
      var _this = this;
      this.serverRoot = options.server || this.defaultServerRoot();
      if ('maxApiServer' in options) {
        this.maxApiServer = options.maxApiServer;
      } else {
        this.maxApiServer = this.defaultMaxApiServer();
      }
    ...
    Client.prototype.authenticate = function(options, callback) {
      ...
      if (!(this.driver || this.authStep === DbxClient.DONE)) {
    ...
    Client.prototype.appInfo = function(appKey, callback) {
      ...
      if ((!callback) && (typeof appKey === 'function')) {
        callback = appKey;
        appKey = this.oauth.credentials().key;
      }
      xhr = new Dropbox.Util.Xhr('GET', this.urls.appsInfo);
      xhr.setParams({
        app_key: appKey
      });
      return this.dispatchXhr(xhr, function(error, appInfo) {
    ...
    Client.prototype.isAppDeveloper = function(userId, appKey, callback) {
      ...
      xhr = new Dropbox.Util.Xhr('GET', this.urls.appsCheckDeveloper);
      xhr.setParams({
        app_key: appKey,
        uid: userId
      });
      return this.dispatchXhr(xhr, function(error, response) {
        if (response) {
    ...
    Client.prototype.hasOauthRedirectUri = function(redirectUri, appKey, callback) {
      ...
      xhr = new Dropbox.Util.Xhr('GET', this.urls.appsCheckRedirectUri);
      xhr.setParams({
        app_key: appKey,
    ...
    Client.prototype.setupUrls = function() {
      this.apiServer = this.chooseApiServer();
      return this.urls = {
    ...
    Client.prototype.chooseApiServer = function() {
      var serverId, serverNumber;
      serverNumber = Math.floor(Math.random() * (this.maxApiServer + 1));
      serverId = serverNumber === 0 ? '' : serverNumber.toString();
      return this.serverRoot.replace('$', serverId);
    };
    ...
    Client.prototype.defaultServerRoot = function() {
    ...
    Client.prototype.defaultMaxApiServer = function() {
      return 30;
    };
    ...
    Client.prototype.computeCredentials = function() {
      ...
      if (this.serverRoot !== this.defaultServerRoot()) {
        value.server = this.serverRoot;
      }
      if (this.maxApiServer !== this.defaultMaxApiServer()) {
        value.maxApiServer = this.maxApiServer;
      }
  ...
  (function() {
    var add32, arrayToBase64, crypto, hmacSha1, requireError, rotateLeft32, sha1, stringToArray, _base64Digits;
